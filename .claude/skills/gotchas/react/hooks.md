...
### Clerk `baseTheme` from `@clerk/themes` silently ignored under Clerk v7

**Tried:** `<ClerkProvider appearance={{ baseTheme: dark }}>` with `dark` imported from `@clerk/themes`. UserButton popover and SignIn card stayed white regardless of `<html>.dark` class. Hardcoding `isDark = true` didn't help — the theme just wasn't being applied.
**Broke:** Version mismatch. `@clerk/nextjs@7.0.4` resolves `@clerk/shared@4.3.2` at the top level. `@clerk/themes@2.4.57` is built against `@clerk/shared@3.47.3` and ships a nested copy under `node_modules/@clerk/themes/node_modules/@clerk/shared/`. The `BaseTheme` shape changed between v3 and v4, so the `dark` object is silently rejected when Clerk v7 inspects `appearance.baseTheme`. No console warning. `@clerk/themes@3.x` exists only as canary (`3.0.0-canary-core3.*`) at time of writing.
**Fix:** Don't use `baseTheme` from `@clerk/themes@2.x` with Clerk v7. Define dark theme via `appearance.variables` directly (uses Clerk's own appearance API, no cross-package object): `{ variables: { colorBackground: "#232528", colorForeground: "#ffffff", colorPrimary: "#5e6ad2", ... } }`. Mirror values from `.dark` block in `src/app/globals.css` for consistency. See `src/components/clerk-provider-wrapper.tsx`. Use `useSyncExternalStore` to subscribe to `<html>.dark` class (correct first-render value, no flash) and `key={isDark ? "dark" : "light"}` on `<ClerkProvider>` to force clean remount on theme switch.
**Watch out:** Clerk's `appearance.variables` does NOT accept CSS `var(--…)` strings — values must be hardcoded color literals. When updating `.dark` in `globals.css`, sync the wrapper too. Also: Clerk loads its UI from a CDN — `elements` keys for the UserButton popover are only `userButtonPopoverActions` and `userButtonPopoverFooter`. Individual action buttons use `userButtonPopoverActionButton`; section dividers use `borderTop`/`"&:not(:last-child)"` patterns since there are no dedicated divider keys.
