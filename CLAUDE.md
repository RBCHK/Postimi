...
**`e2e` is path-filtered + advisory, not blocking.** Both `ci` and `e2e` live in `.github/workflows/ci.yml` as parallel jobs (no `needs:` between them — happy-path PR time stays at max(ci, e2e) ≈ 5 min). The `e2e` job depends on a tiny `changes` job that uses `dorny/paths-filter` to skip e2e for test-only / docs-only / `.claude/**` / `prisma/seeds/**` / `design-handoff/**` PRs. Branch protection requires only the `ci` context (`gh api repos/<owner>/<repo>/branches/main/protection --jq '.required_status_checks'` confirms). A skipped `e2e` job does not block merge. **If you ever flip `e2e` to required**, you must pair it with a skip-stub job — otherwise GitHub blocks merge on test-only PRs (skipped ≠ pass for required checks).

**After PR merge**: `git checkout main && git pull --rebase` to sync local main. Production deploy starts automatically (Vercel runs `prisma migrate deploy && next build` per `vercel.json`'s `buildCommand`). **Don't synchronously block on it** — check status when naturally needed (before next merge, end of session) via `gh api repos/<owner>/<repo>/deployments --jq '.[0]'` then `gh api repos/.../deployments/<id>/statuses`. Sentry + Vercel email on failure, so silent breakage is impossible. Sitting on `gh run watch` for 5 minutes after every merge wastes 60+ min/day across a PR-heavy session — that pattern was the single biggest velocity killer pre-2026-04.
...
IMPORTANT: **Parallel-PR workflow** for independent tasks. When the next task touches code that doesn't overlap with the in-flight PR, push the current PR and immediately start the next on a new branch off `main` — don't wait for merge. CI + your own work proceed in parallel; you save 10-15 min/PR vs. the serial pattern. When the in-flight PR's CI returns green, merge it, then `git rebase main` (or `git pull --rebase` after switching back) on the next branch. Conflicts are rare for disjoint work; if they happen, resolve and force-push. Don't run this pattern when the next task structurally depends on the in-flight PR (e.g. you're about to consume an export that's in the unmerged branch).
