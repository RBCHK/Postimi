...
**`e2e` is path-filtered + advisory, not blocking.** `.github/workflows/e2e.yml` runs only when paths that affect runtime change (not on test-only / docs-only / `.claude/**` / `prisma/seeds/**` / `design-handoff/**` PRs). Branch protection requires only the `ci` context (`gh api repos/<owner>/<repo>/branches/main/protection --jq '.required_status_checks'` confirms). A skipped `e2e` workflow does not block merge. **If you ever flip `e2e` to required**, you must pair it with a skip-stub job — otherwise GitHub blocks merge on test-only PRs (skipped ≠ pass for required checks).
