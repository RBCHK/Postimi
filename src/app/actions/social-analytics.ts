...
export interface SocialAnalyticsSummary {
  ...
  avgEngagementRate: number; // percent, 2 decimals

  latestFollowers: number | null;
  netFollowerGrowth: number; // over the period
...
async function _getSocialAnalyticsDateRange(
  userId: string,
  platform: Platform
...
async function _getSocialAnalyticsSummary(
  ...
  // Posts per calendar day — uses postedAt, not dailyStats, so platforms
  // without daily-stats rows (LinkedIn CSV followers-only, Threads) still
  ...
  // Followers series: prefer snapshots; compute net growth over the
  // period as (last − first) rather than summing deltaFollowers, which
  // can drift if a day is missing from the snapshot set.
...
export async function getSocialAnalyticsSummaryInternal(
  userId: string,
  platform: Platform,
  from: Date,
  to: Date
