...
/**
 * Get valid LinkedIn API credentials for a specific user.
 * Auto-refreshes if token is within 5 days of expiry (access tokens last 60 days).
 * Returns null if user has no connected LinkedIn account.
 */
export async function getLinkedInApiTokenForUserInternal(
  userId: string
  ...
  // Token expiring within 5 days — refresh
...
/** Get LinkedIn profile data for composer preview */
...
/** Disconnect LinkedIn account for current user */
...
export async function getLinkedInApiTokenForUser(): Promise<LinkedInApiCredentials | null> {
  const userId = await requireUserId();
  return getLinkedInApiTokenForUserInternal(userId);
}
